...
def mahalanobis(x=None, data=None, cov=None):
    ...
    if not is_pos_def(cov):
        cov = cov + np.identity(cov.shape[0])
...
def bald(
    ...
    Attributes:
        ...
        T: Number of MC dropout iterations aka training iterations,
        training: If False, run test without MC dropout. (default=True)
    """# random_subset includes the selected indices
...
def var_ratios(
    model, X_pool: np.ndarray, n_query: int = 10, T: int = 100, training: bool = True, clusters=None):
    ...
    if clusters is not None: 
        query_idx = []
        random_subset_clusters = clusters[random_subset]
        for ind in range(n_query):
            cluster_locations = np.where(random_subset_clusters == ind)[0]
            idx = (-acquisition[cluster_locations]).argsort()[0]
            query_idx.append(random_subset[cluster_locations[idx]])
